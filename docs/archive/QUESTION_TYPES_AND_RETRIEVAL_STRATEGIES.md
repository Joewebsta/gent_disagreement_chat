...
The current RAG system uses a **one-size-fits-all** retrieval strategy that works well for general topical questions but struggles with episode-scoped, speaker-specific, temporal, comparative, and factual queries. This document categorizes the full range of question types and identifies the specific retrieval methods needed for each.

**Key Finding:** The system already classifies query intent but doesn't use it to adapt retrieval strategy. Implementing intent-aware routing could dramatically improve answer quality across all question types.
...
- **Structured query parsing** to extract episode constraints (episode numbers, date ranges, "latest")
...
- Actually handles this reasonably well with multi-query search
- BUT: Returns only top 10 segments across ALL episodes
- May miss important discussions if >10 episodes discuss the topic
- Episode grouping helps but still limited by `max_docs=10`
...
- Query expansion adds host names ("Ricky", "Brendan"), but this is weak
...
- No awareness that this is a comparative query (intent IS classified as "comparative" but not used!)
...
- **Intent-aware retrieval**: Detect comparative intent using existing classification
...
**Implementation Priority:** MEDIUM-HIGH - Leverages existing intent classification, high user value
...
- Query expansion helps but not guaranteed to catch all name variants
...
- **General topical questions** - Multi-query search with HyDE + expansion works well
- **Query enhancement** - HyDE and expansion improve recall
- **Episode grouping** - Hierarchical organization by episode is clear
- **Adaptive thresholding** - Ensures minimum context without overload
- **Speaker attribution** - Maintains speaker metadata through pipeline
...
- **Intent classification not used** - Computes intent but doesn't adapt strategy

### Core Issue

The system uses a **one-size-fits-all** retrieval strategy:

1. Enhance query (same approach for all question types)
2. Multi-query vector search (3 variants: original, HyDE, expanded)
3. Top 10 by similarity
4. Group by episode
5. Pass to LLM

This works reasonably for general topical questions but **fails to leverage**:

- Available metadata (episode_number, speaker name, date_published)
- Query intent classification (already computed but unused!)
- Different ranking strategies for different intents
...
- **Intent classification**: Use existing classification, expand categories as needed
...
- Existing OpenAI intent classification (already implemented)
...
- Episode aggregation logic (already partially implemented)
...
1. Add episode number extraction from queries
2. Add speaker name extraction from queries
3. Implement filtered vector search (Method 1)
4. Route episode-scoped questions to filtered search
5. Route speaker-specific questions to filtered search
6. Add adaptive document limits

**Estimated Effort:** 1-2 weeks
**Impact:** HIGH - Solves common pain points

---

### Phase 2: Intent-Aware Retrieval (High Priority)

**Goal:** Use existing intent classification to adapt retrieval strategy.

**Tasks:**

1. Implement retrieval strategy router
2. Add comparative question handling (Method 3: Multi-faceted)
3. Add date parsing and temporal filtering (Method 6)
4. Implement adaptive parameters per intent
5. Add diversity-aware ranking (Method 5: MMR)
...
The current RAG system has a solid foundation but uses a one-size-fits-all approach that limits its effectiveness across diverse question types. The biggest opportunity is leveraging already-available metadata (episode, speaker, date) and already-computed intent classification to route queries to appropriate retrieval strategies.
...
3. Intent-aware routing using existing classification (HIGH impact, MEDIUM effort)

**Long-term improvements:** 4. Hybrid search for factual queries (MEDIUM impact, MEDIUM effort) 5. Two-stage retrieval for broad topical questions (MEDIUM impact, MEDIUM effort) 6. Pre-computed summaries for corpus-level questions (MEDIUM impact, HIGH effort)

The evaluation framework already in place (evaluation_runner.py, automated_metrics.py) provides the tooling to measure improvements systematically as these enhancements are implemented.
