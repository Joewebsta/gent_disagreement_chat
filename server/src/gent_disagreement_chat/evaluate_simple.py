...
TEST_QUESTIONS = [
    ...
    # ("What topics did Brendan and Ricky cover in episode 181?", EPISODE_SCOPED),
    ("What did Lydia DePhillis discuss about Nepal in episode 182?", EPISODE_SCOPED),
    # (
    ...
    ("What does Ricky think about gerrymandering and redistricting?", SPEAKER_SPECIFIC),
    # (
    ...
    # ("What were they predicting about the economy in late August 2024?", TEMPORAL),
    # (
    ...
    #     FACTUAL_LOOKUP,
    # ),
    # ("When did they discuss the Bureau of Labor Statistics firing?", FACTUAL_LOOKUP),
    # ("Which episode talked about electric vehicles in Nepal?", FACTUAL_LOOKUP),
    ("Have they ever discussed the Posse Comitatus Act?", FACTUAL_LOOKUP),
...
def evaluate():
    ...
    for question, category in TEST_QUESTIONS:
        ...
        if not segments:
            ...
            # Still generate LLM response to see how it handles no context
            print(f"\n{'─' * 80}")
            print("LLM RESPONSE (no context):")
            print(f"{'─' * 80}\n")

            response_chunks = []
            for chunk in rag_service.ask_question_text_stream(question):
            ...
            print(f"\n{'─' * 80}")
            print("Is this response good? (y/n/notes): ", end="")
            feedback = input()
    ...
    for category, items in by_category.items():
        ...
        if items_with_segments:
            print(
                f"  Avg segments: {sum(i['num_segments'] for i in items_with_segments) / len(items_with_segments):.1f}"
            )
            print(
                f"  Avg episodes: {sum(i['num_episodes'] for i in items_with_segments) / len(items_with_segments):.1f}"
            )
            print(
                f"  Avg similarity: {sum(i['avg_similarity'] for i in items_with_segments) / len(items_with_segments):.3f}"
            )
        else:
